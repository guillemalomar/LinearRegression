...
def gradient_descent(input_file):
    ...
    for i in range(0, 100000):
        cost = cost_function(input_data, thetas)
        print(cost)
        for ind, theta in enumerate(thetas):
            new_thetas.append(theta - (step_size * cost))
        thetas = list(new_thetas)
        new_thetas = []
    print(thetas)


def cost_function(input_data, thetas):
    total = 0
    for ind, training_example in input_data.items():
        training_data = [1]
        training_data.extend(training_example[:-1])
        training_example_hypothesis = hypothesis(training_data, thetas)
        total += (training_example[-1] - training_example_hypothesis)
    result = -(2/len(input_data.keys())) * total
    return result


def hypothesis(training_example, thetas):
    total = 0
    for ind, theta in enumerate(thetas):
        total += training_example[ind] * theta
    return total
'''
